...
// GetPlatform returns the platform configuration, preferring the new top-level field
// and falling back to the deprecated external.platform location.
func (c *Config) GetPlatform() (*vclusterconfig.Platform, error) {
	// Prefer new top-level field
	if c.Platform != nil {
		return c.Platform, nil
	}

	// Fallback to deprecated external.platform
	return c.GetPlatformConfig()
}

// GetSnapshots returns the snapshots configuration from the new top-level field.
func (c *Config) GetSnapshots() *vclusterconfig.Snapshots {
	return c.Snapshots
}

// GetDeletion returns the deletion configuration from the new top-level field.
func (c *Config) GetDeletion() *vclusterconfig.Deletion {
	return c.Deletion
}
